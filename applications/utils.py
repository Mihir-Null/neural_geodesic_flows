...
from core.models import TangentBundle_single_chart_atlas as TangentBundle
...
from applications.configs import get_optimizer

from core.training import train

# get the relevant inference methods
from core.inference import trajectory_model_analyis, trajectory_model_visualization
...
def load_dataset(name, size=None, random_selection=False, key=jax.random.PRNGKey(0)):
    ...
    if {"inputs", "targets", "times"} <= keys:
        ...
        arrays = tuple(
            jnp.array(loaded_data[k]) for k in ["inputs", "targets", "times"]
        )
...
def create_dataloader(
    ...
    arrays, mode = load_dataset(
        name=dataset_name, size=dataset_size, random_selection=random_selection, key=key
    )
...
def perform_training(
    ...
    if config.continue_training:
        ...
        model = TangentBundle(
            dim_dataspace=config.dim_dataspace,
            dim_M=config.dim_M,
            psi=psi_NN,
            phi=phi_NN,
            g=g_NN,
    ...
    optimizer = get_optimizer(
        name=config.optimizer_name, learning_rate=config.learning_rate
    )
