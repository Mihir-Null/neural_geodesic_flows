"""
Training script for Riemannian (SPD) NGF baseline on flat Minkowski trajectories.
Note: The data is pseudo-R, but this serves as an SPD baseline comparison.
"""
...
wandb.init(project="Neural geodesic flows",
           group="Minkowski flat",
           dir=PATH_LOGS)

################################ choose all hyperparameters and neural networks used ################################

config = get_wandb_config(
    train_dataset_name="minkowski_flat_train",
    test_dataset_name="minkowski_flat_test",
    model_name="minkowski_rngf",
    dim_dataspace=4,  # (t, x, vt, vx)
    dim_M=2,
