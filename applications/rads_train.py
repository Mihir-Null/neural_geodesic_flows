"""
Training script for Riemannian (SPD) NGF baseline on AdS2 trajectories.
"""
...
wandb.init(project="Neural geodesic flows",
           group="AdS2 trajectories",
           dir=PATH_LOGS)

################################ choose all hyperparameters and neural networks used ################################

config = get_wandb_config(
    train_dataset_name="ads2_train",
    test_dataset_name="ads2_test",
    model_name="ads2_rngf",
    dim_dataspace=6,  # (X,V) in ambient R^{2,1}
    dim_M=2,
    psi_arguments={"in_size": 6, "out_size": 4, "hidden_sizes_x": [64, 64]},
    phi_arguments={"in_size": 4, "out_size": 6, "hidden_sizes_x": [64, 64]},
    g_arguments={
        "dim_M": 2,
        "hidden_sizes": [64, 64],
    },
